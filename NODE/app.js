...
const JWT_SECRET = "ashdfigfbaskjhdfalohalkjshbaisdfug[]jashgdvfk2aius@sdbfaskjbvkas";
const demographics = require('./demographics');
const questionaire = require('./Questionnare');
...
app.get('/demographics/:email', async (req, res) => {
    try {
        ...
        if (!udemo) {
            return res.status(404).json({ message: "No demo data found for this email" });
        }
...
app.get('/Personality/:email', async (req, res) => {
    try {
        const uPersonality = await Personality
            .findOne({ email: req.params.email })
            .sort({ _id: -1 })
            .limit(1);

        if (!uPersonality) {
            return res.status(404).json({ message: "No personality data found for this email" });
        }
...
app.post("/change-password", async (req, res) => {
    ...
    try {
        ...
        if (newPassword === oldPassword) {
            return res.json({ status: "error", message: "New password must be different from old password" });
        }
...
app.get('/questionaire/:email', async (req, res) => {
    try {
        ...
        if (!userQuestionnaire) {
            return res.status(404).json({ message: "No questionnaire data found for this email" });
        }
...
const mongoUrl = "mongodb+srv://nobita9699:xgFi1COLriKTuji7@cluster0.uh0srwc.mongodb.net/?retryWrites=true&w=majority";
...
app.post("/ForgotPassword", async (req, res) => {
    ...
    try {
        ...
        var transporter = nodemailer.createTransport({
            service: "gmail",
            auth: {
                user: "nobita9699@gmail.com",
                pass: "dmkaqzjwoqnxjfcs",
            },
        });

        var mailOptions = {
            from: "nobita9699@gmail.com",
            to: email,
